...
public class AlgCursorAdapter extends CursorRecyclerAdapter<RecyclerView.ViewHolder> implements DialogListener {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        ItemAlgListBinding binding = ItemAlgListBinding.inflate(inflater, parent, false);

        return new AlgHolder(binding);
    }
    ...
    public void handleTime(final AlgHolder holder, final Cursor cursor) {
        ...
        holder.binding.name.setText(pName);
        holder.binding.progressBar.setProgress(pProgress);
        holder.binding.cube.setCubeState(pState);

        // If the subset is PLL, it'll need to show the pll arrows.
        if (cursor.getString(1).equals("PLL")) {
            holder.binding.pllArrows.setImageDrawable(AlgUtils.getPllArrow(mContext, pName));
            holder.binding.pllArrows.setVisibility(View.VISIBLE);
        }
    ...
    static class AlgHolder extends RecyclerView.ViewHolder {
        final ItemAlgListBinding binding;
