...
public class TimeCursorAdapter extends CursorRecyclerAdapter<RecyclerView.ViewHolder> implements DialogListener {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        ItemTimeListBinding binding = ItemTimeListBinding.inflate(inflater, parent, false);
    ...
    private void handleTime(final TimeHolder holder, final Cursor cursor) {
        ...
        holder.binding.date.setText(new DateTime(pDate).toString(mDateFormatSpec));

        if (isSelected(mId))
            holder.binding.card.setBackground(selectedCardBackground);
        else
            holder.binding.card.setBackground(cardBackground);

        holder.binding.root.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if (isInSelectionMode)
                    toggleSelection(mId, holder.binding.card);
                else if (! isLocked()) {
                    setIsLocked(true);
        ...
        holder.binding.timeText.setText(Html.fromHtml(PuzzleUtils.convertTimeToString(pTime, PuzzleUtils.FORMAT_SMALL_MILLI)));
        holder.binding.penaltyText.setTextColor(ContextCompat.getColor(mContext, R.color.red_material));

        switch (pPenalty) {
            case PuzzleUtils.PENALTY_DNF:
                holder.binding.timeText.setText("DNF");
                holder.binding.penaltyText.setVisibility(View.GONE);
                break;
            case PuzzleUtils.PENALTY_PLUSTWO:
                holder.binding.penaltyText.setText("+2");
                holder.binding.penaltyText.setVisibility(View.VISIBLE);
                break;
            default:
                holder.binding.penaltyText.setVisibility(View.GONE);
                break;
        ...
        if (! pComment.equals("")) {
            holder.binding.commentIcon.setVisibility(View.VISIBLE);
        } else {
            // This else is needed because the view recycles.
            holder.binding.commentIcon.setVisibility(View.GONE);
        }
    ...
    static class TimeHolder extends RecyclerView.ViewHolder {
        final ItemTimeListBinding binding;
