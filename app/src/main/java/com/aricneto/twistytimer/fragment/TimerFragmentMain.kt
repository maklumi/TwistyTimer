...
public class TimerFragmentMain extends BaseFragment implements OnBackPressedInFragmentListener, DialogListenerMessage {
    ...
    private FragmentTimerMainBinding binding;
            ...
            switch (intent.getAction()) {
                ...
                case ACTION_TIMER_STARTED:
                    ...
                    binding.actionbar.toolbar.animate()
                            .translationY(-binding.actionbar.toolbar.getHeight())
                            .alpha(0)
                            .setDuration(mAnimationDuration);

                    binding.tabView.animate()
                             .translationY(binding.tabView.getHeight())
                             .alpha(0)
                             .setDuration(mAnimationDuration);
                    break;
    ...
    private void updatePuzzleSpinnerHeader() {
        history = false;
        updateHistorySwitchItem();
        binding.actionbar.puzzleCategory.setText(currentPuzzleCategory.toLowerCase());
        if (currentTimerMode.equals(TIMER_MODE_TRAINER))
            binding.actionbar.puzzleName.setText(getString(R.string.title_trainer, currentPuzzleSubset.name()));
        else
            binding.actionbar.puzzleName.setText(PuzzleUtils.getPuzzleNameFromType(currentPuzzle));
    }
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        binding.root.setBackground(ThemeUtils.fetchBackgroundGradient(mContext, ThemeUtils.getPreferredTheme()));
        ...
        binding.pager.setUserInputEnabled(pagerEnabled);

        // Menu bar background
        if (Prefs.getBoolean(R.string.pk_menu_background, false)) {
            binding.actionbar.toolbar.setCardBackgroundColor(Color.TRANSPARENT);
            binding.actionbar.toolbar.setCardElevation(0);
        }

        viewPagerAdapter = new NavigationAdapter(this);
        binding.pager.setAdapter(viewPagerAdapter);
        binding.pager.setOffscreenPageLimit(NUM_PAGES - 1);

        new TabLayoutMediator(binding.mainTabs, binding.pager, (tab, position) -> {
            switch (position) {
                case TIMER_PAGE:
        ...
        binding.mainTabs.setTabIconTint(AppCompatResources.getColorStateList(mContext, R.color.tab_color));
        tabStrip = ((LinearLayout) binding.mainTabs.getChildAt(0));

        binding.mainTabs.getBackground().setColorFilter(ThemeUtils.fetchAttrColor(mContext, R.attr.colorTabBar), PorterDuff.Mode.SRC_IN);
    ...
    private void setupPage(int pageNum) {
        ...
        switch (pageNum) {
            case TIMER_PAGE:
                if (binding.actionbar.navButtonHistory != null) {
                    binding.actionbar.navButtonHistory.animate()
                            .withStartAction(() -> binding.actionbar.navButtonHistory.setEnabled(false))
                            .alpha(0)
                            .setDuration(200)
                            .withEndAction(() -> {
                                // navButtonHistory may already be destroyed by the time we get
                                // to the end action, so we have to check if it still exists
                                if (binding != null && binding.actionbar.navButtonHistory != null) binding.actionbar.navButtonHistory.setVisibility(View.GONE);
                            })
                            .start();
                }
                break;
            ...
            case GRAPH_PAGE:
                if (binding.actionbar.navButtonHistory != null) {
                    binding.actionbar.navButtonHistory.setVisibility(View.VISIBLE);
                    binding.actionbar.navButtonHistory.animate()
                            .withStartAction(() -> binding.actionbar.navButtonHistory.setEnabled(true))
                            .alpha(1)
                            .setDuration(200)
                            .start();
                }
    ...
    private void handleHeaderSpinner() {
        ...
        binding.actionbar.puzzleSpinner.setOnClickListener(v -> {
    ...
    protected class NavigationAdapter extends FragmentStateAdapter {

        private final SparseArray<Fragment> fragments = new SparseArray<>();

        public NavigationAdapter(Fragment fragment) {
            super(fragment);
        }
        ...
        public Fragment createFragment(int position) {
            ...
            switch (position) {
                case TIMER_PAGE:
                    ...
                    break;
                case LIST_PAGE:
                    fragment = TimerListFragment.newInstance(
                            currentPuzzle, currentPuzzleCategory, history);
                    break;
                case GRAPH_PAGE:
                    fragment = TimerGraphFragment.newInstance(
                            currentPuzzle, currentPuzzleCategory, history);
                    break;
                default:
                    fragment = TimerFragment.newInstance(PuzzleUtils.TYPE_333, "Normal", TIMER_MODE_TIMER, TrainerScrambler.TrainerSubset.OLL);
            }
        ...
        public boolean dispatchOnBackPressedInFragment() {
            ...
            for (int i = 0; i < fragments.size(); i++) {
                ...
                if (fragment instanceof OnBackPressedInFragmentListener && fragment.isAdded()) {
