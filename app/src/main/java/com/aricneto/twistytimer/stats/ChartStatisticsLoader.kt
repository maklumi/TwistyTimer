...
class ChartStatisticsLoader(
     ...
     */
    private var mChartStats: ChartStatistics? = null

    /**
     * A wrapper around the loaded chart statistics. This is required to allow the same
     * `ChartStatistics` instance to be used for successive loads while ensuring that the
     * `LoaderManager` sees a different object delivered, otherwise it would not invoke
     * `onLoadFinished` on the activity or fragment waiting for the updated data.
     */
    private var mLoadedData: Wrapper<ChartStatistics?>? = null
     ...
     */(
         ...
         */
        private val mLoader: ChartStatisticsLoader
    ) : BroadcastReceiver() {
        /**
        ...
        override fun onReceive(context: Context?, intent: Intent) {
            ...
            when (Objects.requireNonNull<String?>(intent.getAction())) {
                ...
                ACTION_TIMES_MOVED_TO_HISTORY ->                     // If session times were moved to the history, AND THE FULL HISTORY IS BEING
                ...
                ACTION_SESSION_TIMES_SHOWN -> if (mLoader.resetForSelection(true)) {
                    if (DEBUG_ME) Log.d(TAG, "  Changed to session times. Will reload!")
                    mLoader.onContentChanged()
                } // else was already showing "current session" chart, so do nothing.

                ACTION_HISTORY_TIMES_SHOWN -> if (mLoader.resetForSelection(false)) {
     ...
     * are created, the "wrapper" content will be set to `null` to indicate that the new
     ...
     * 
     * @return
     * `true` if the selection for the solve times was changed and new chart statistics
     * were created (and will need to be loaded from the database); or `false` if the
     * selection is the same as that already loaded (and the old chart statistics are still
     * valid).
     */
    private fun resetForSelection(isForCurrentSessionOnly: Boolean): Boolean {
        if (mChartStats == null
            ...
            mChartStats = if (isForCurrentSessionOnly)
            ...
            mLoadedData = wrap<ChartStatistics?>(null)
    ...
    private val isForCurrentSessionOnly: Boolean
        /**
         ...
         * 
         * @return
         * `true` if the loaded chart statistics cover only the solve times for the current
         * session; or `false` if the full history of times is loaded. If no statistics have
         * been loaded, the result will be `true`.
         */
        get() =// NOTE: "true" if nothing is loaded, as it suits the case for ACTION_TIMES_MOVED_TO_HISTORY
            // in the broadcast receiver. If "true", it will trigger a re-load, which makes sense.
            mLoadedData!!.isEmpty() || mChartStats!!.isForCurrentSessionOnly
        ...
        if (!mLoadedData!!.isEmpty()) {
            ...
            if (solve != null) {
                if (solve.penalty == PuzzleUtils.PENALTY_DNF) {
                    ...
                    mChartStats!!.addTime(solve.time.toLong(), solve.date)
                }
    ...
    override fun onReset() {
        ...
        mLoadedData = wrap<ChartStatistics?>(null) // "null" flags "not loaded" state.
        mChartStats!!.reset()
     ...
     */
    override fun loadInBackground(): Wrapper<ChartStatistics?>? {
        ...
        if (DEBUG_ME) Log.d(
            TAG, String.format(
                "  Loaded ChartStatistics in %,d ms.",
    ...
    companion object {
         ...
         */
        private val TAG: String = ChartStatisticsLoader::class.java.getSimpleName()
    }
}
